...
async function main() {
	let dbSqlite = await sqlite.open('db/word.sqlite', {Promise});
	...
	await dbMysql.execute('DROP TABLE IF EXISTS word');
	...
	let stmt = await dbSqlite.prepare('SELECT CAST(hash AS TEXT) hash, a, b, c FROM word');
	let row;
	let n = 0;
	...
	while (row = await stmt.get()) {
		second++;
		total++;
		await dbMysql.query('INSERT INTO word VALUES (?,?,?,?)',
			[
				row.hash,
				row.a, row.b, row.c
			]);
	...
	await dbMysql.query('CREATE INDEX word_idx ON word (hash)');
